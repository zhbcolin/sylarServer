...
void test_yaml() {
    YAML::Node root = YAML::LoadFile("/home/colinzhb/CLionProjects/sylar/bin/conf/log.yml");
    print_yaml(root, 0);
    //YLAR_LOG_INFO(SYLAR_LOG_ROOT()) << root;
}
...
#define XX_PM(g_var, prefix) \
    ...
    XX_PM(g_person_map, "class.amp before");
    SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "before: " << g_person_vec_map->toString();

    YAML::Node root = YAML::LoadFile("/home/colinzhb/CLionProjects/sylar/bin/conf/test.yml");
    sylar::Config::LoadFromYaml(root);
